...
export default function SquareCheckoutModal({ isOpen, onClose, cart, totalAmount, onPaymentSuccess }) {
  ...
  useEffect(() => {
    if (showPaymentForm && !card && typeof window !== 'undefined') {
  ...
  const initializeSquarePayments = async () => {
    ...
    if (!appId || !locationId || appId === 'your_app_id_here' || locationId === 'your_location_id_here') {
      console.error('Square credentials not configured properly');
